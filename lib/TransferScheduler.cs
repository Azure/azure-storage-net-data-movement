    ...
    internal sealed class TransferScheduler : IDisposable
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex) when (ex is StorageException || (ex is AggregateException && ex.InnerException is StorageException))
            {
                ...
                {
                    job.Logger?.Error(storageException,$"Inner exception threw {nameof(OperationCanceledException)} DmLib will cancel the transfer. Message: {storageException.Message} RequestId: {storageException?.RequestInformation?.ServiceRequestID}");
